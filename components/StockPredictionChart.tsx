...
export default function StockPredictionChart() {
  ...
  const fetchHistoricalData = async (stockSymbol: string) => {
    try {
      const response = await fetch(
        `http://localhost:8000/api/historical/${stockSymbol}`,
      );
      ...
      if (result.data && result.data.length > 0) {
        ...
        const lastHourData: HistoricalDataPoint[] = result.data
          .slice(-60)
          .map((item: any) => ({
  ...
  const fetchRecentPredictions = async (stockSymbol: string) => {
    try {
      const response = await fetch(
        `http://localhost:8000/api/recent-predictions/${stockSymbol}?minutes=15`,
      );
      ...
      if (result.predictions && result.predictions.length > 0) {
        const predictions: PredictionDataPoint[] = result.predictions.map(
          (item: any) => ({
            timestamp: new Date(item.timestamp),
    ...
    ws.onmessage = (event) => {
      try {
        ...
        setLivePredictions((prevData) => {
          const updated: PredictionDataPoint[] = [
            ...prevData,
            {
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-5 gap-4 mb-6">
          ...
          <div className="bg-white rounded-lg shadow-md p-6">
            ...
            <p
              className={`text-2xl font-bold ${priceChangeNum >= 0 ? "text-green-600" : "text-red-600"}`}
            >
              {priceChangeNum >= 0 ? "+" : ""}
              {priceChange}%
            </p>
        ...
        <div className="mt-4 text-center text-sm text-gray-600">
          {historicalData.length > 0 && (
            <p>
              ...
              {recentPredictions.length > 0 &&
                `, ${recentPredictions.length} recent predictions`}
              {livePredictions.length > 0 &&
                `, ${livePredictions.length} live predictions`}
            </p>
          )}
        </div>
      </div>
